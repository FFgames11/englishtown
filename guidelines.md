...
- Per-property placement and sizing for art-backed structures should live in `styles.css` under `.structure-{propertyId}[data-level="..."]` rules, matching the existing `church`, `bank`, `townHall`, `ancestralHouse`, `library`, `commercialCenter`, `marketHall`, `bakery`, and `schoolhouse` patterns.
